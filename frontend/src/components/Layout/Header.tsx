// ─── Header component with Web3 and SSO authentication integrations ──────────
...
const PAGE_META: Record<NavPage, { title: string; subtitle: string }> = {
  academy:      { title: 'Web3 Developer Academy',  subtitle: 'Multichain Smart Contract Curriculum & Hands-on Roadmap' },
  roadmap:      { title: 'Web3 Developer Academy',  subtitle: 'Multichain Smart Contract Curriculum & Hands-on Roadmap' },
  dashboard:    { title: 'Developer Dashboard',     subtitle: 'Track your on-chain learning progress, contracts & metrics' },
  analytics:    { title: 'Cohort Analytics',        subtitle: 'Ecosystem Developer Cohort Activity (May–August 2026)' },
  sandbox:      { title: 'Multi-Chain Web3 Sandbox', subtitle: 'Universal IDE: Write, compile, and deploy Solidity, Rust, Cairo, Move & ink!' },
  forum:        { title: 'Community Forum',         subtitle: 'Ask questions, share smart contracts, and collaborate with Web3 builders' },
  hackathons:   { title: 'Web3 Hackathons & Grants', subtitle: 'Build decentralized applications, innovate, and win ecosystem bounties' },
  careers:      { title: 'Web3 Career Portal',      subtitle: 'Smart contract engineering roles, internships & foundation grants' },
  mentor:       { title: 'AI Mentor Workspace',     subtitle: 'Get real-time smart contract code reviews and security feedback' },
  certificates: { title: 'Web3 Credentials',        subtitle: 'View and export your verified blockchain credentials' },
  subscriptions: { title: 'Subscription Plans',     subtitle: 'Choose a plan to unlock premium mentorship and credentials' },
  about:        { title: 'About the Academy',       subtitle: 'Why the Academy exists and how it empowers Web3 developers' },
};
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const getHeaderMeta = (page: NavPage, _loggedIn: boolean) => {
    return PAGE_META[page] || { title: 'Web3 Academy', subtitle: 'Multichain Smart Contract Engineering' };
  };
          ...
          {authType ? (
            <div className="auth-profile">
              ...
              {authType === 'github' && !progress?.wallet_address && (
                <button className="btn btn--secondary btn--xs header-link-btn" onClick={onLinkWallet} title="Link Web3 Wallet" style={{ fontSize: '0.7rem', padding: '3px 8px', marginLeft: 8 }}>
                  🦊 Link Wallet
                </button>
              )}
            ...
            <div className="auth-buttons" style={{ display: 'flex', gap: '8px' }}>
              <button className="btn btn--secondary btn--sm" onClick={onLoginWallet} title="Connect Web3 Wallet">
                🦊 Connect Wallet
              </button>
